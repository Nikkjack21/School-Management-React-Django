...
const TeachersLIst = () => {
  ...
  useEffect(() => {
    axios
      .get("http://127.0.0.1:8000/all-teachers")
  ...
  return (
    <div>
      ...
      {list.map((item, id) => (
        <div key={id}>
          <div className="flex border w-48 flex-col justify-center hover:scale-105  shadow-lg rounded-xl ">
            <img
              src={item.image}
              alt=""
            ...
            <div className="space-y-4 text-center divide-y divide-gray-700 ">
              <div className="my-2 space-y-1">
                <h2 className="text-base font-semibold ">{item.user.first_name} {item.user.last_name}</h2>
                <p className="px-5 text-base  dark:text-gray-600">{item.class_number.class_number}</p>
              </div>
              <div className="flex justify-center pt-2 py-2 space-x-4 align-center">
                <div >
                  <BsPencil className="cursor-pointer" onClick={()=>setid((item.id),(item.user))}                                                                                                                                               />
                </div>
                <div>
                  <MdDeleteOutline />
                </div>
